...
class Orchestrator:
    ...
    def _log_cycle(
        ...
        # パイプ区切りのテーブル形式で出力
        print(
            f"{timestamp:>5.1f}s | {f_dist:>4.0f}mm | {rf_dist:>5.0f}mm | {lf_dist:>5.0f}mm | {lr_error:>+5.2f} | {front_flag:>5} | {fork_flag:>5} | {steer:>+5.2f} | {speed:>6.2f}   | {steer_pwm:>7}us | {throttle_pwm:>9}us |  {status_str}"
        )
