...
class VL53L0XConfig:
    ...
    I2C_ADDRESSES: Final[Tuple[int, int, int]] = (0x30, 0x31, 0x32)  # 前、左、左前の順
    
    # センサー設定
    NUM_SENSORS: Final[int] = 3  # センサーの数（前、左、左前）
    MEASUREMENT_TIMING_BUDGET: Final[int] = 20000  # 計測時間バジェット（マイクロ秒）。小さいほど高速だが精度が下がる
    SENSOR_NAMES: Final[Tuple[str, ...]] = ("前", "左", "左前")  # センサー名（順序はXSHUT_PINSと対応）
