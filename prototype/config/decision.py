...
class WallFollowDecisionConfig:
    ...
    FRONT_STEER_GAIN: Final[float] = (
        0.0005  # 前方壁回避の比例ゲイン Kf (steer = clamp(-Kf*(F_th - F), -1, 0))
    )
    FRONT_THRESHOLD_MM: Final[float] = 2000.0  # 前方壁回避が始まる距離閾値 (mm)

    # 左コーナー時の設定
    CORNER_LEFT_SPEED: Final[float] = 0.30  # 左コーナー時の速度
    CORNER_LEFT_STEER_GAIN: Final[float] = (
        0.0004  # 左コーナーの比例ゲイン Kc (steer = clamp(Kc*(LF - LF_th), 0, 1))
    )
